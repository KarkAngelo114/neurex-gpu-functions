...
Napi::Value AccumulateAttentionWeightsGradients_GPU(const Napi::CallbackInfo& info) {
    ...
    for (int i = 0; i < 7; i++) clSetKernelArg(kernel, i, sizeof(cl_mem), &args[i]);
    ...
    for (cl_mem buffer : args) clReleaseMemObject(buffer);
    return weightGrads;
}
...
Napi::Value AccumulateAttentionBiasGrads_GPU(const Napi::CallbackInfo& info) {
    ...
    for (int i = 0; i < 5; i++) clSetKernelArg(kernel, i, sizeof(cl_mem), &args[i]);
